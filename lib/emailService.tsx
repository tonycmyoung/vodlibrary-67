...
export async function sendInvitationEmail({ recipientEmail, inviterName, invitationToken }: SendInvitationEmailParams) {
  const signUpUrl = `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}/auth/sign-up`
...
export async function sendNotificationEmail({
  ...
  const libraryUrl = `${process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000"}`
